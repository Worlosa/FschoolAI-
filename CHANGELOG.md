...
- **File CONTENT extraction (2026-06-08, Phase-2-contents layer b):** new `api/extract.js` (PDF/text → plain text via the existing `pdfjs-dist` dep; verified on a sample PDF) + `/api/extract` dev middleware. `background.js` `extractFileContents()` fetches each synced file's bytes via the LMS session → POSTs to `/api/extract` → PATCHes `files.content_text` (capped, skips already-done + >6MB), wired into both sync paths. `tutor-context` `file_lookup` now selects + surfaces `content_text`. ⚠️ Step-2 (extension fetch→extract→store) is **code-complete but NOT verified e2e** — needs a browser sync + reachable `/api/extract` (set `EXTRACT_URL` to localhost for dev, or deploy it). Caveats: D2L `source_url`s are often viewer pages (may not yield raw PDF bytes); storing contents wants the Phase-0 RLS fix before rollout.
